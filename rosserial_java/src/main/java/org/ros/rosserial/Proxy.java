...
class Proxy {
	 ...
	 * 
	 * The {@link Publisher}s are parameterized with {@link Object} so that we
	 * can handle arbitrary message definition classes (e.g. new style or old
	 * style messages).
	 */
	 ...
	 *            the TopicInfo message describing the topic
	 */
	public void registerSubscriber(int topicId, String topicName,
			String messageType, MessageListener<?> listener) {
		Subscriber<?> subscriber = node.newSubscriber(topicName, messageType,
				listener);
		getSubscribers().put(topicId, subscriber);
		node.getLog().info(
				"Subscribed to " + topicName + " (" + messageType + ").");
	}
